...
class StockController(AccountsController):
    ...
    def update_bin(self, sle: dict):
        """Update Bin (real-time stock quantity) - Enhanced with projected_qty"""
        table = get_table_name("Bin")
        item_code = sle.get('item_code')
        warehouse = sle.get('warehouse')
        actual_qty = float(sle.get('actual_qty', 0))
        reserved_qty = float(sle.get('reserved_qty', 0))
        
        # Check if Bin exists
        existing = db.sql(
            f"SELECT name, actual_qty, reserved_qty, ordered_qty FROM {table} WHERE item_code = ? AND warehouse = ?",
            (item_code, warehouse),
            as_dict=True
        )
        ...
        if existing:
            bin_data = existing[0]
            new_actual = float(bin_data.get('actual_qty', 0)) + actual_qty
            new_reserved = float(bin_data.get('reserved_qty', 0)) + reserved_qty
            new_ordered = float(bin_data.get('ordered_qty', 0))
            
            # Calculate projected_qty like ERPNext
            # projected = actual + ordered - reserved
            projected_qty = new_actual + new_ordered - new_reserved
            
            db.sql(
                f"UPDATE {table} SET actual_qty = ?, reserved_qty = ?, projected_qty = ? WHERE name = ?",
                (new_actual, new_reserved, projected_qty, bin_data['name'])
            )
        else:
            # Create new Bin with calculated projected_qty
            ...
            bin_doc.reserved_qty = reserved_qty
            bin_doc.ordered_qty = 0
            bin_doc.projected_qty = projected_qty
            bin_doc.save()
